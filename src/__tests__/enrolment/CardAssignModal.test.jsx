...
describe('CardAssignModal', () => {
  beforeEach(() => { onSelect.mockClear(); onClose.mockClear(); vi.mocked(addExtraCard).mockClear() })
...
describe('CardAssignModal — Add a new card form', () => {
  ...
  test('confirming new card calls addExtraCard with form data', async () => {
    const user = userEvent.setup()
    renderModal()
    await user.click(screen.getByLabelText('Add a new card'))
    await user.type(screen.getByLabelText('Name'), 'Jane Smith')
    ...
    await user.type(screen.getByLabelText('CVV'), '123')
    await user.click(screen.getByRole('button', { name: /confirm/i }))
    expect(addExtraCard).toHaveBeenCalledWith(expect.objectContaining({
      name: 'Jane Smith',
      panSuffix: '1111',
      expiry: '12/28',
    }))
  ...
  test('confirming new card calls onSelect with the new card id', async () => {
    const user = userEvent.setup()
    renderModal()
    await user.click(screen.getByLabelText('Add a new card'))
    await user.type(screen.getByLabelText('Name'), 'Jane Smith')
    ...
    await user.type(screen.getByLabelText('CVV'), '123')
    await user.click(screen.getByRole('button', { name: /confirm/i }))
    const newCard = vi.mocked(addExtraCard).mock.calls[0][0]
    expect(onSelect).toHaveBeenCalledWith(newCard.id)
  })
})
