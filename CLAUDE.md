...
- Use `mcp__headroom__headroom_compress` on any large tool output before reasoning over it (file reads >100 lines, search results, test output, build logs). Retrieve with `mcp__headroom__headroom_retrieve` only when full detail needed.

Mobile-responsive React SPA — digital card issuance, spend dashboard, transit transaction history, top-up flow.

## Working Directory

App lives in `mobile-simulator/`. All dev commands run from there.
...
│   ├── ConcessionContext.jsx    # Global enrolled bool + concessions[] array + resetConcession
...
│   │   ├── SheerIDModal.jsx         # Clicking red TEST MODE banner autofills all form fields
│   │   └── CardAssignModal.jsx  # "Add a new card" tile expands inline form (Name, Card Number, Expiry, CVV)
│   │                            #   On confirm: addExtraCard(card) + onSelect(card.id)
│   │                            #   "Create a new digital card" tile: selectable, confirm → onClose (no-op for now)
│   ├── transaction/
│   │   ├── DateRangeFilter.jsx
│   │   ├── TransactionList.jsx
│   │   └── TransactionRow.jsx
...
│                                #   Nav bar (hamburger+lang+bell) is sticky top-0 z-10 — never moves on scroll
│                                #   Title "Concession and Eligibility" + icon render directly below the sticky nav bar
│   └── enrolment/
...
7. ConcessionPage (enrolled): shows ALL concession cards (one per enrolment), `Active (N)` count, "Start new concession enrolment" button at `bottom-[34px]`
8. ConcessionPage (unenrolled): shows "Click here to start enrolment" CTA
9. Both states: nav bar (hamburger + lang toggle + bell) is `sticky top-0 z-10`; title + icon appear directly below it

## Concession Context (`ConcessionContext.jsx`)

- `enrolled: bool` — gates enrolled UI
- `concessions: Array<{ group, card, enrolledAt }>` — accumulating list; each `setConcessionData` call appends
- `setEnrolled(bool)`, `setConcessionData(group, card)`, `resetConcession()` — clears both enrolled + concessions[]
...
- `ConcessionPage` maps over `concessions[]` — shows `Active (N)` count + one card per entry
...
- No bottom nav bar — navigation is in HamburgerMenu (slide-in from left)
- HamburgerMenu BASE_ITEMS: Home, Cards, Concession, Profile
...
- Android side controls panel (shown when `showControls=true`): Home + $2/$5/$10 Trip + **Pass Trip** (green, value 0 — displays "Pass Used" in Recent activity, does not advance fare cap bar) + **Payment Issue** (red — calls `showPaymentIssue()`)
- `androidPaymentIssueStore.js` — subscribable pub-sub (`useSyncExternalStore`): `showPaymentIssue`, `hidePaymentIssue`, `resetPaymentIssue`, `isPaymentIssueVisible`, `usePaymentIssueStore`
- Payment Issue banner renders at top of `TransitPassDetailScreen` scroll region (above Littlobus header). Design: light red `#FDECEC` bg, red `!` badge, "You have a payment issue" title, "Fix issue" + `ChevronRight`, white `X` dismiss button top-right (`aria-label="Dismiss payment issue"`) calls `hidePaymentIssue()`. `SettingsScreen` Search settings also calls `resetPaymentIssue()`
...
- Trips: `data/androidTripsStore.js` — module-level subscribable store. Exports `addTrip`, `removeTrip(index)`, `resetTrips`, `getTrips`, `subscribe`, `useTripsStore` (via `useSyncExternalStore`). Trip shape: `{ name, amount: string, value: number, date }` — `value` drives fare cap math, `amount` is display string. Drives Recent Activity on `TransitPassDetailScreen`
...
All trip buttons add a trip row to Recent Activity on `TransitPassDetailScreen` and contribute toward the fare cap. Each trip carries a numeric `value` used for cap accumulation plus a display `amount` string.

- **Home** — `goHome()` — returns to Android home screen from any app
- **$2.00 Trip** — `addTrip({ name: 'Station 1', amount: '$2.00', value: 2, date })`
- **$5.00 Trip** — `addTrip({ name: 'Station 3', amount: '$5.00', value: 5, date })`
- **$10.00 Trip** — `addTrip({ name: 'Station 2', amount: '$10.00', value: 10, date })`

## Fare Cap Logic (`TransitPassDetailScreen`)

- `FARE_CAP = 10` (dollars). Daily free-travel threshold
- `totalSpend = trips.reduce((s, t) => s + (t.value ?? 0), 0)` — recomputes on add/remove
- `pctFill = Math.min(100, (totalSpend / FARE_CAP) * 100)` — bar width, hard-capped at 100%
- `remaining = Math.max(0, FARE_CAP - totalSpend)`
- `dateStr = new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric' }) + ' 03:00'` — cap-reset date is end of today at 03:00
- Text states (exclusive):
  - no trips → `"Start riding to see your fare cap progress towards free rides"`
  - `totalSpend >= FARE_CAP` → `"You have free travel until <dateStr>"`
  - otherwise → `"Spend $X.XX more to have free travel <dateStr>"`
- Bar fill div has `data-testid="fare-cap-bar-fill"` for test access
- Removing a trip via clicking the row recomputes text + bar automatically (UI derives from `trips`)

## Recent Activity — Trip Row Removal

- Each trip row in Recent Activity is a `<button>` with `aria-label="Remove <station name>"`
- `onClick` calls `removeTrip(index)` from `androidTripsStore`
- Removal notifies subscribers; fare cap text + bar + row list all re-derive
